  ...
  <div tag="div" class="articleList articleListMod-3 er-carouseindex" name="slide-fade" id="articleList" :style="{height:imgHeight+'px'}" >
    ...
    <div id="packageAll" class="er-carouseindex-con" @mouseover="clearAuto" @mouseout="slideAuto">
      <div class="er-carouseindex-bar" v-show="isOrNotCircle">
        <div v-for="(item,index) in imgList" @mouseup="clearAuto" :key="index" class="er-carouseindex-circle" @click="circleClick(dex)" :class="{circleSelected:dex===indexCircle}">
        </div>
      </div>
      <div id="imageAll" class="er-carouseindex-item" :style="{transform:translateX,transition:transFlag?transitionTime:''}">
        <div class="articleList-box er-carouseindex-box" v-for="(list,index) in imgLisShow" :key="index" :style="{width:imgWidth+'%'}"
             style="max-height:50%;">
          <slot :item="list"></slot>
        </div>
      </div>
...
export default
{
  ...
  methods: {
    ...
    imgView: function () {
      var _this = this
      _this.imgList = new Array(['http://f12.baidu.com/it/u=1981748892,3031683197&fm=72'])
      for (var i = 0; i < 3; i++) {
